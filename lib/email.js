...
<div style="max-width:560px;margin:0 auto;padding:28px 16px 40px">
  ...
  <div style="background:#FFFFFF;border-radius:14px;padding:30px 28px;border:1.5px solid #DDD7CE;margin-bottom:16px">
    ...
    <p style="margin:0 0 14px;font-size:15px;color:#5A5450;line-height:1.75;">
      Your Blueprint is attached as a PDF. The insecurity underneath your triggers, what you actually need, the scripts, the 7-day plan — all of it was built from what you shared. It's yours.
    </p>
    ...
    <div style="background:#FDF0E6;border-left:3px solid #E07A3A;border-radius:0 10px 10px 0;padding:16px 20px;margin:20px 0">
      ...
      <p style="margin:0 0 7px;font-size:14px;color:#1E1E1E;line-height:1.65;font-weight:600">Then read what's actually been happening and what you really need underneath it.</p>
  ...
  const emailPayload = { from: FROM, replyTo: 'ash@coupleseducator.com', to, subject, html };
  if (pdfBuffer) {
...
<div style="max-width:560px;margin:0 auto;padding:28px 16px 40px">
  ...
  const { data, error } = await resend.emails.send({ from: FROM, replyTo: 'ash@coupleseducator.com', to, subject, html });
  if (error) throw new Error(`Resend error: ${JSON.stringify(error)}`);
  return data;
}
...
// ── Failure alert ─────────────────────────────────────────────────────────────
// A paid order can fail after payment (generation, PDF, or send) and nothing in
// the normal flow surfaces it — the sale notification rides at the end of the
// same chain, so a delivery failure silences its own alert. This is independent.
async function sendFailureAlert({ stage, email, detail, recovered = false }) {
  const when = new Date().toISOString().replace('T', ' ').slice(0, 19) + ' UTC';
  const tone = recovered ? { bg: '#1E3526', label: 'Recovered automatically', icon: '✅' }
                         : { bg: '#7A2B22', label: 'Needs your attention',     icon: '⚠️' };
  const html = `<!DOCTYPE html>
<html><head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background:#F7F3EE;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif">
<div style="max-width:560px;margin:0 auto;padding:28px 16px 40px">
  <div style="background:${tone.bg};border-radius:14px;padding:22px 24px;margin-bottom:16px">
    <p style="margin:0 0 6px;font-size:11px;letter-spacing:2px;color:rgba(255,255,255,.65);font-weight:800;text-transform:uppercase">${tone.icon} ${tone.label}</p>
    <p style="margin:0;font-size:19px;font-weight:800;color:#fff;line-height:1.3">${recovered ? 'A stuck order was delivered' : 'A paid order did not deliver'}</p>
  </div>
  <div style="background:#fff;border:1.5px solid #DDD7CE;border-radius:14px;padding:22px 24px">
    <table style="width:100%;border-collapse:collapse;font-size:14px;color:#1E1E1E">
      <tr><td style="padding:6px 0;color:#6B6560;width:96px">Customer</td><td style="padding:6px 0;font-weight:700">${email || 'unknown'}</td></tr>
      <tr><td style="padding:6px 0;color:#6B6560">Stage</td><td style="padding:6px 0;font-weight:700">${stage}</td></tr>
      <tr><td style="padding:6px 0;color:#6B6560">When</td><td style="padding:6px 0;font-weight:700">${when}</td></tr>
    </table>
    <div style="background:#F7F3EE;border-radius:10px;padding:12px 14px;margin-top:14px">
      <p style="margin:0;font-size:12.5px;color:#5A5450;line-height:1.6;font-family:ui-monospace,Menlo,monospace">${String(detail || '').slice(0, 400)}</p>
    </div>
    <p style="margin:16px 0 0;font-size:13.5px;color:#5A5450;line-height:1.7">
      ${recovered
        ? 'The recovery sweep found this order and sent the blueprint. No action needed, but worth knowing it happened.'
        : 'The sweep runs every 15 minutes and will retry automatically if a blueprint was saved. If this repeats, the order may need sending by hand.'}
    </p>
  </div>
</div></body></html>`;
  ...
  const { error } = await resend.emails.send({
    from: FROM,
    to: 'ash@coupleseducator.com',
    subject: `${tone.icon} BondBlueprint: ${recovered ? 'recovered' : 'delivery failed'} — ${email || 'unknown'}`,
    html,
  });
  if (error) throw new Error(error.message || 'alert send failed');
}

module.exports = { sendFailureAlert, sendBlueprintEmail, sendOwnerNotificationEmail, sendClaimReminderEmail, sendAbandonedCartEmail };
