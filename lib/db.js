...
// Orders that were paid for and generated, but whose delivery email never went out.
// This happens when the process dies mid-flight (deploy, crash, restart) after the
// blueprint is saved but before the PDF/email step completes — the customer's money
// is taken and nothing is ever sent. Safe to retry: sending is idempotent per row
// because markEmailSent flips the flag.
async function getUndeliveredPurchases() {
  const now       = Date.now();
  // Skip anything younger than 3 min so we never race a request still processing.
  const settled   = new Date(now -  3 * 60 * 1000).toISOString();
  // Don't reach back further than 30 days — old rows are historical test noise.
  const horizon   = new Date(now - 30 * 24 * 60 * 60 * 1000).toISOString();
  ...
  return (data || []).filter(p => {
    if (TEST_EMAILS.includes((p.email || '').toLowerCase())) return false;
    const bd = p.blueprint_data;
    // Needs a real generated blueprint — not a placeholder or a half-written row.
    if (!bd || bd._pending === true || bd._processing === true) return false;
    return !!(bd.title && Array.isArray(bd.scripts) && bd.scripts.length);
  });
}
...
// Leads who entered their email but never completed a purchase — eligible for abandoned cart nudge.
// Looks back 1–72 hrs so we catch recent dropoffs without re-emailing old cold leads.
async function getAbandonedLeads() {
  const now    = new Date();
  const cutoff = new Date(now.getTime() -  1 * 60 * 60 * 1000).toISOString(); // at least 1hr ago
  const oldest = new Date(now.getTime() - 72 * 60 * 60 * 1000).toISOString(); // no older than 72hrs

  // Get all leads in that window that haven't been reminded yet
  const { data: leads, error } = await supabase
    .from('leads')
    .select('email, name, quiz_data')
    .lte('created_at', cutoff)
    .gte('created_at', oldest)
    .not('quiz_data->_ac_reminded', 'eq', 'true');  // _ac_reminded flag in quiz_data JSONB

  if (error) throw error;

  // Filter out leads that have a completed purchase (they already bought)
  ...
  const buyers = new Set((purchases || []).map(p => p.email));
  return (leads || []).filter(l => !buyers.has(l.email));
}

async function markAbandonedCartReminderSent(email) {
  const { data: lead } = await supabase
    .from('leads').select('quiz_data').eq('email', email).single();
  const updated = { ...(lead?.quiz_data || {}), _ac_reminded: true };
  await supabase.from('leads').update({ quiz_data: updated }).eq('email', email);
}
