...
function generateBlueprintPdf(blueprint, { name = '', attachmentStyle = '', partnerStyle = '' } = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Subtitle
    const coverSub = blueprint.subtitle || 'Your personalized attachment reading — built from everything you shared.';
    ...
    const introText = blueprint.coverIntro ||
      `You are not too much. Your nervous system is doing exactly what it was built to do — and this blueprint will show you how to respond from a place of security instead of panic. What follows is built specifically from what you shared.`;
    const introY = sY + 66;
    ...
    if (blueprint.citations?.length) {
      ...
      doc.y += 6;
      sectionRule('Research Behind This Reading', C.muted);
    ...
    const closingMsg = blueprint.closingMessage ||
      `You are not too much. You are not broken. You are a person with an attachment history — and now you have a map. What you do next matters more than what happened before.`;
    ...
    txt('— Ashley, CouplesEducator.com', ML + 20, clY + closingH - 22, {
