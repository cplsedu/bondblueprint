...
function buildHTML(blueprint, { name = '', attachmentStyle = 'Anxious', partnerStyle = 'Dismissive-Avoidant' } = {}) {
  ...
  // Cycle Conversation page data
  const cycleAns     = blueprint.cycleAnswers || {};
  const cycleInsight = blueprint.cycleInsight || [];
  const cycleQA = [
    { q: 'What triggered you?',                                                        a: cycleAns.trigger  },
    { q: 'What did you feel emotionally, and what did you feel in your body?',         a: cycleAns.feelings },
    { q: 'Why do you feel they made the choice that triggered you?',                   a: cycleAns.story    },
    { q: 'What did you do to handle or cope with that trigger?',                       a: cycleAns.cope     },
  ];
  const cycleAnswersHTML = cycleQA.filter(x => x.a).map(x =>
    `<div class="cyc-card"><div class="cyc-q">${esc(x.q)}</div><div class="cyc-a">&ldquo;${esc(x.a)}&rdquo;</div></div>`
  ).join('');
...
/* CYCLE CONVERSATION */
.cyc-card { background: var(--card); border: 1.5px solid var(--border); border-radius: 12px; padding: 9px 14px; margin-bottom: 6px; }
.cyc-card .cyc-q { font-size: 10px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; color: var(--teal); margin-bottom: 4px; }
.cyc-card .cyc-a { font-size: 12px; font-weight: 600; color: var(--text); line-height: 1.6; font-style: italic; }
.cyc-insight { background: var(--lt-navy); border-left: 3px solid var(--navy); border-radius: 0 10px 10px 0; padding: 11px 16px; margin: 10px 0; }
.cyc-insight p { font-size: 12px; font-weight: 600; color: var(--text); line-height: 1.7; margin-bottom: 8px; }
.cyc-insight p:last-child { margin-bottom: 0; }
...
.cyc-rules { background: var(--lt-rose); border: 1.5px solid var(--rose); border-radius: 12px; padding: 12px 16px; margin-top: 10px; }
.cyc-rules .rules-title { font-size: 10px; font-weight: 800; letter-spacing: 2px; text-transform: uppercase; color: var(--rose); margin-bottom: 8px; }
.cyc-rule { display: flex; gap: 10px; margin-bottom: 5px; }
.cyc-rule .dot { color: var(--rose); font-weight: 800; flex-shrink: 0; }
.cyc-rule .txt { font-size: 11.5px; font-weight: 600; color: var(--text); line-height: 1.6; }
...
<div class="cover">
  ...
  <div class="toc-grid">
    ...
    <div class="toc-item"><div class="toc-num">4</div><div class="toc-text">The Cycle Conversation</div></div>
    <div class="toc-item"><div class="toc-num">5</div><div class="toc-text">What You Actually Need</div></div>
...
<!-- ═══ PAGE 4B: THE CYCLE CONVERSATION ═════════════════════════════════ -->
<div class="section-page">
  <div class="page-header">
    <div class="page-header-num">4</div>
    <div class="page-header-content">
      <div class="page-header-label ac-teal">Map Your Cycle</div>
      <div class="page-header-title">The Cycle Conversation</div>
      <div class="page-header-sub">Every conflict runs the same loop: trigger &rarr; feeling &rarr; story &rarr; coping move. You mapped your side. Now invite theirs.</div>
    </div>
  </div>

  ${cycleAnswersHTML ? `
  <div class="section-label">Your Side of the Loop, In Your Own Words</div>
  ${cycleAnswersHTML}
  ` : `
  <div class="section-label">First, Map Your Side of the Loop</div>
  <div class="cyc-card"><div class="cyc-q">Ask yourself these four questions about your most recent conflict</div><div class="cyc-a">What triggered you? What did you feel emotionally, and what did you feel in your body? Why do you feel they made the choice that triggered you? What did you do to handle or cope with that trigger?</div></div>
  `}

  ${cycleInsight.length ? `
  <div class="cyc-insight">
    ${cycleInsight.map(p => `<p>${esc(p)}</p>`).join('')}
  </div>
  ` : ''}

  <div class="section-label">Now Flip It: Ask Them the Same Four Questions</div>
  <div class="cyc-ask"><div class="cyc-ask-num">1</div><div class="cyc-ask-text">What triggered you?</div></div>
  <div class="cyc-ask"><div class="cyc-ask-num">2</div><div class="cyc-ask-text">What did you feel emotionally, and what did you feel in your body, once you were triggered?</div></div>
  <div class="cyc-ask"><div class="cyc-ask-num">3</div><div class="cyc-ask-text">Why do you feel I made the choice that triggered you?</div></div>
  <div class="cyc-ask"><div class="cyc-ask-num">4</div><div class="cyc-ask-text">What did you do to handle or cope with that trigger?</div></div>

  <div class="cyc-rules">
    <div class="rules-title">Before You Start This Conversation</div>
    <div class="cyc-rule"><span class="dot">&bull;</span><span class="txt">Ask them first: &ldquo;When would be a good time to talk?&rdquo; Never ambush the conversation.</span></div>
    <div class="cyc-rule"><span class="dot">&bull;</span><span class="txt">Keep the space free of judgment, criticism, and blame. You are mapping the loop together, not building a case.</span></div>
    <div class="cyc-rule"><span class="dot">&bull;</span><span class="txt">If what you hear is hurtful, remember: it is a reflection of their inner state, not your value.</span></div>
    <div class="cyc-rule"><span class="dot">&bull;</span><span class="txt">With real honesty comes true connection. That is the whole point of this page.</span></div>
  </div>
</div>

<!-- ═══ PAGE 4C: THE NEED BENEATH THE TRIGGER ═══════════════════════════ -->
<div class="section-page">
  <div class="page-header">
